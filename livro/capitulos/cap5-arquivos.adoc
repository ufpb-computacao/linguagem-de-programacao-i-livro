...
Neste primeiro teste vamos comparar a escrita num arquivo normal e a
escrita no arquivo `NUL`, no Windows:

....
> echo "oi" > arquivo.txt <1>
> type arquivo.txt <2>
oi <3>
> echo "oi" > nul <4>
> type nul <4>
...
<1> Invoca o comando `echo` para escrever a mensagem `oi` e
    redireciona a saída ao arquivo `arquivo.txt` que será criado ou
    sobrescrito, caso exista.
<2> Utiliza o comando `type` para ler o conteúdo do arquivo.
<3> O conteúdo do arquivo lido.
...
`CON`? Reflita um pouco sobre o resultado esperado depois teste: `type
con`.
...
NOTE: Este tipo de arquivo especial não existe no Windows, todos os
arquivos especiais neste sistema estão relacionados aos dispositivos.
