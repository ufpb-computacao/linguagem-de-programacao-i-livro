...
Implementamos a leitura do arquivo através de três funções, vamos ver
cada uma delas a seguir.

A implementação fará referências a diversas funções como: <<fopen>>,
<<fclose>>, <<fread>> e <<fseek>>. Além disso, os arquivos são
referenciados através de um ponteiro para `FILE` (`File *arquivo`).  

===== Implementando lerConteudoDoArquivo com fopen e fclose
...
----
<1> Variável `conteudo` irá salvar o conteúdo do arquivo.
...
<1> Nesta implementação estamos lendo o conteúdo inteiro do arquivo,
    para isso precisamos saber qual o tamanho total do arquivo. Vamos
    implementar a função `lerTamanhoDoArquivo` que será responsável
    por obter o tamanho do arquivo.
<2> Com o tamanho do arquivo salvo na variável `tamanhoDoArquivo` será
    necessário alocar o espaço para guardar todo o conteúdo do arquivo
    e mais um byte zero, para indicar o final do String.
<3> Com o buffer `conteudo` inicializado com zeros, chamamos a função
    ...
    `conteudo`.
<4> Por fim, retornamos o buffer contendo o conteúdo lido.

==== Descobrindo o tamanho do arquivo

Para descobrir o tamanho do arquivo, vamos utilizar as funções `fseek`
e `ftell`:
