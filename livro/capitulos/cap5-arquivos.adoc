...
Por alguma razão nosso teste está falhando, apesar da implementação
simples aparentar estar correta.

Esta é uma boa oportunidade para *((Depurar))* a execução do programa.
A depuração será abordada em um capítulo a parte: <<cap_depuracao>>.

TIP: Apesar de não ser um pré-requisito para continuação desta
capítulo, convidamos você a ler o <<cap_depuracao>> antes de continuar
para conhecer o processo de depuração.
