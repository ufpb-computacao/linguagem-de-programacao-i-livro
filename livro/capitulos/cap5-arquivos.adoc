...
depuração dos programas. Caso a função receba uma expressão com o valor
...
----
<1> Inclusão da biblioteca `stdbool.h`, que define o tipo `bool`. Este
    tipo pode ser utilizado como retorno de expressões lógicas. Além
    disso também estão definidos macro `true` com o valor `1` e
    `false` com o valor `0`.
<2> A função `fopen` retorna um ponteiro para `File` se conseguir
    abrir o arquivo, caso contrário retorna `NULL`. Esta função
    tem o propósito de verificar o resultado da chamada àquela
    função.
...
<4> Para criar um string com o conteúdo do arquivo, primeiro
    ...
    `calloc`, além de alocar espaço na memória, inicializa todo o
    espaço alocado com `0` (zeros). Nem sempre é possível alocar
    espaço para armazenar o espaço do arquivo inteiro, caso ele seja
    grande demais.
