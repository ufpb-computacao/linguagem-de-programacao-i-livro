...
enquanto não chegou no final. A implementação desta funcionalidade
ocorreu na função que se segue:
...
acentuadas e estão passando com esta implementação. Para fazê-los
falharem nós precisaremos atualizá-los da seguinte forma: 

Atualizamos o arquivo `musica-completa-traduzida.txt` substituindo
todas as vogais acentuadas pelas suas respectivas representações na
lingua do i: 
...
tradução, será necessário atualizar todas as vogais acentudas nele.
...
.Representação de carácteres acentuados em UTF-8
|====
...
Vamos modificar a função `traduzParaLingaDoI` responsável por traduzir
uma linha:
...
char* traduzParaLingaDoI(char* mensagemOriginal){
	...
	for(int i=0; i<tamanhoDaMensagem; i++){
		char caracterDaMensagemOriginal = mensagemOriginal[i];
		if
		(podeSerVogalAcentuada(caracterDaMensagemOriginal)){ <1>
...
<1> Nossa primeira modificação consiste em verificar se o primeiro
    byte lido corresponde ao início de uma letra acentuada. Se ela não
    for, então seguirá o fluxo normal, sendo traduzida pela função que
    traduz carácteres de um byte, e adicionada ao string que
    corresponde à tradução.
<2> Caso o primeiro byte lido seja o início da vogal acentuda,
    chamaremos a função `traduzCaracteresAcentuados` que deverá ler
    mais um carácter na posição atual e salvá-los no string
    `traducao`. Teoricamente o carácter acentuado porderia possuir
    mais de 2 bytes, portanto esta unção precisa retornar um
    incremento para ser utilizado em `i`, para garantir a consistência
    da posição de leitura.
<3> Atualizamos o valor de `i` com o incremento necessário. É
    ...
    apenas um carácter for lido em `traduzCaracteresAcentuados`.

 }else if(s==-94 or s==-86 or s==-82 or s==-76 or s==-69){
   traducao[posicao+1] = -82; // î
