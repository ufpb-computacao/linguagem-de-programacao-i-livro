...
* Analisar problemas e reconhecer os campos necessários para utilizar
  nos registros;
...
Em uma disciplina na qual os alunos possuem *duas notas*, e precisamos registrar e 
...
----
include::{local}/reg_aluno_pseudo.txt[]
----
...
.Registro em Pseudocódigo
----
include::{local}/reg_pessoa_pseudo.txt[]
----

:srcfile: reg_pessoa.c
...
.Registro em Pseudocódigo
----
include::{local}/reg_ponto_matematica_pseudo.txt[]
----
...
Geralmente ele utiliza uma tabela, na qual há o nome do produto e o seu código. Para a nossa
...
.Registro em Pseudocódigo
----
include::{local}/reg_produto_supermercado_pseudo.txt[]
----

:srcfile: reg_produto.c
...
utilizado  como o identificador.footnote:[Exceto quando o 
...
.Registro em Pseudocódigo
----
include::{local}/reg_conta_pseudo.txt[]
----
...
.Registro em Pseudocódigo
----
include::{local}/reg_cliente_pseudo.txt[]
----

:srcfile: reg_cliente.c
...
:srcfile: reg_aluno_exemplo.c
:saida:   reg_aluno_exemplo_simulacao.txt
...
.Utilização do registro Produto em C
====
:srcfile: reg_produto_exemplo.c
:saida:   reg_produto_exemplo.txt
...
WARNING: Percebam que quando atribuímos um valor de texto aos campos do tipo *char[]*,
...
.Utilização do Registro Ponto
====

:srcfile: reg_ponto_exemplo.c
:saida:   reg_ponto_exemplo_simulacao.txt
...
----
include::{local}/reg_aluno_pseudo.txt[]
include::{local}/calculo_das_medias_pseudo.txt[]
----
<1> Imprime a média calculada.

.Implementação de calculo_das_medias.c
====
:srcfile: calculo_das_medias.c
:saida:   calculo_das_medias_simulacao.txt
...
----
include::{local}/reg_pessoa_pseudo.txt[]
include::{local}/imc_calculo_pseudo.txt[]
----
<1> O ler o campo *identificador* de Pessoa (CPF).
<2> Pesquisa pelo registro Pessoa identificado pelo CPF lido.

.Implementação de imc_calculo.c
====
:srcfile: imc_calculo.c
:saida:   imc_calculo_simulacao.txt
...
----
include::{local}/reg_ponto_matematica_pseudo.txt[]
include::{local}/ponto_proximo_pseudo.txt[]
----
...
.Implementação de ponto_proximo.c
====
:srcfile: ponto_proximo.c
:saida:   ponto_proximo_simulacao.txt
...
----
include::{local}/reg_produto_supermercado_pseudo.txt[]
include::{local}/prob_supermercado_pseudo.txt[]
----

.Implementação de supermercado.c
====
:srcfile: supermercado.c
:saida:   supermercado_simulacao.txt
...
include::{local}/reg_conta_pseudo.txt[]
include::{local}/reg_cliente_pseudo.txt[]
include::{local}/prob_conta_bancaria_pseudo.txt[]
----

.Implementação de conta_bancaria.c
====

:srcfile: conta_bancaria.c
:saida:   conta_bancaria_simulacao.txt
...
Após todos estes programas, agora vamos ver uma técnica não utilizada ainda,
...
.Programa em C
====
:srcfile: reg_atribuicao.c
:saida:   reg_atribuicao_simulacao.txt
...
----
include::{local}/reg_triangulo_pseudo.txt[]
----

.Composição de registro em C - Triângulo
====
:srcfile: reg_triangulo.c
.Código fonte
{online}/{srcfile}[{local}/{srcfile}]
...
.Composição de registro em Pseudocódigo
----
include::{local}/reg_infopessoal_pseudo.txt[]
----

.Composição de registro em C - Informação Pessoal
====
:srcfile: reg_infopessoal.c
...
** Coleção de variáveis referenciadas por um mesmo nome
...
Na <<sec_registro_programas>> vimos como implementar diversos programas em
pseudocódigo e em C.
