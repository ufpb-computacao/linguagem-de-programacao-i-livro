...
No início do Capitulo 1, foi discutido que o conhecimento de linguagens 
...
essa variável (posições `0xCB22` e `0xCB23`). Esse espaço de memória é 
...
variáveis antes do seu uso. Veja no <<ex_variavel_nao_inicializada>> que, quando não se 
...
código foi criado, compilado e executado o valor de `conta` 
foi 624.756.000,00!footnote:[Você iria trabalhar a vida inteira para 
pagar essa conta de luz!]
Ou seja, um valor totalmente aleatório e que muda a cada vez que a memória 
RAM do computador é inicializada. O correto é inicializar a variável antes de 
...
[[ex_variavel_nao_inicializada]]
.Programa para demonstrar valor de variável não inicializada
====

:srcfile: variavel_nao_inicializada.c
:saida:   variavel_nao_inicializada.out
...
variável na memória, ou seja, o endereço de memória. O <<ex_uso_operador_endereco>> 
...
[[ex_uso_operador_endereco]]
.Programa para demonstrar uso do Operador de Endereço
====
:srcfile: operador_de_endereco.c
:saida:   operador_de_endereco.data
...
Com o intuito de sedimentar o conceito de ponteiros, o <<ex_uso_ponteiro>> 
mostra como um ponteiro é usado para manipular o conteúdo da variável 
...
[[ex_uso_ponteiro]]
.Usando ponteiro para alterar variável
====

:srcfile: usando_ponteiro.c
:saida:   usando_ponteiro.out
...
O <<ex_fsoma_pares>> mostra um programa 
que calcula a soma dos números pares entre 0 e 200. A etapa de cálculo da 
soma é feita na função `f_soma`, a qual é chamada na linha 10 dentro da 
...
[[ex_fsoma_pares]]
.Calcula soma dos números pares entre 0 e 200
====
:srcfile: f_soma.c
.Código fonte
{online}/{srcfile}[{local}/{srcfile}]
...
<<fig_pilha_de_execucao>>.  Resumidamente, a pilha de execução funciona da seguinte
forma. Cada variável local de uma função é colocada na
...
programa (linhas 6 a 9 do programa), a <<fig_pilha_executando>>(b)
mostra a definição e inicialização das variáveis `soma` e `PAR` na
pilha. Com a chamada da função (linha 10), a
...
Para melhor entender esse debate, o <<ex_calcular_soma_e_subtracao>> mostra um 
...
[[ex_calcular_soma_e_subtracao]]
.Calcular a soma e subtração de dois números inteiros
====

:srcfile: subtracao-passagem-por-valor.c
...
mostrada no <<ex_passagem_incorreta>>.

[[ex_passagem_incorreta]]
.Implementação incorreta de passagem de parâmetros
====
:srcfile: passagem_incorreta.c
:saida:   passagem_incorreta.out
...
Como se sabe, e já foi alertado no texto, _esse código não funciona_. Serão 
...
possível resolver o problema do <<ex_passagem_incorreta>>. Isso é feito no 
código do <<ex_passagem_por_referencia>>.

[[ex_passagem_por_referencia]]
.Passagem de valores por referência
====

:srcfile: passagem_por_referencia.c
:saida:   passagem_por_referencia.out
...
include::{local}/{srcfile}[]
----
<1> Protótipo da função cabeçalho definindo receber dois números
    inteiros e dois ponteiros como para inteiros como parâmetros.
<2> Chamada da função calculadora, similar ao programa anterior, mas
    agora passa o endereço das variáveis `sub` e `soma`.
<3> Implementação da função com os ponteiros, similar ao cabeçalho;
    `ptr1` recebe o endereço de memória de `sub` e `ptr2` o de `soma`.
...
A implementação da função `calculadora` utiliza dois inteiros (`x` e
...
cenário, quando a função `calculadora` é chamada, os
