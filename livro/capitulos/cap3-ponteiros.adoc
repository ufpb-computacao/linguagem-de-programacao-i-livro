...
// FIXME: Esta capítulo não abordou Alocação Dinâmica de Memória
...
image::images/cap3/memoria-soma.jpg[]
...
.Código fonte
{gitrepo}/blob/master/livro/capitulos/code/cap3/variavel_nao_inicializada.c[code/cap3/variavel_nao_inicializada.c]

[[variavel_nao_inicializada]]
[source, c]
.Programa para demonstrar valor de variável não inicializada
----
include::code/cap3/variavel_nao_inicializada.c[]
----
...
.Código fonte
{gitrepo}/blob/master/livro/capitulos/code/cap3/operador_de_endereco.c[code/cap3/operador_de_endereco.c]

[source, c]
.Programa para demonstrar uso do Operador de Endereço
----
include::code/cap3/operador_de_endereco.c[]
----

.Saída da execução do programa
....
include::code/cap3/operador_de_endereco.data[]
....
...
nome que está sendo dado ao ponteiro e `*` é o *s((operador de indireção))* e é 
usado para se definir um ponteiro. Lê-se como ``ptr é um ponteiro para um float''.

Como o ponteiro contém um endereço de memória, diz-se que ele aponta para 
aquela posição de memória. A <<fig_memoria_declaracao_inicializacao>> ilustra a definição de um ponteiro 
com nome `ptr`, que é utilizado para apontar para a variável `float soma`. Note 
que na linha 4 `ptr` é declarado. Até esse momento não existe nenhuma 
relação entre o ponteiro `ptr` e a variável `soma`. Na linha 5, `ptr` é associado 
...
.Ilustração de declaração e inicialização de um ponteiro.
image::images/cap3/memoria-declaracao-e-inicializacao.jpg[]
...
Outra maneira de se inicializar ponteiros é associando-o diretamente a uma 
variável. Para isso o operador de endereço `&` deve ser utilizado. A <<fig_inicializacao_ponteiro>>(a) 
ilustra que, ao declarar um ponteiro, o computador não aloca automaticamente 
...
.Inicialização de um ponteiro. a) Forma errada. b) Forma correta.
image::images/cap3/inicializacao-ponteiro.jpg[]
...
para qual está apontando. Na linha 7, o ponteito `pt` é criado e inicializado 
na linha 8, i.e., é associado à variável inteira `tot`. Note que nas linhas 10 
e 11 o conteúdo da variável `tot` é impresso na tela usando a própria 
variável (linha 10) e usando o ponteiro (linha 11). Para você se convencer de 
que o ponteiro `pt` realmente armazena o endereço de memória da variável `tot`, 
na linha 13 é impresso na tela o endereço de memória da variável `tot` e na 
linha 14 o valor armazenado pelo ponteiro, ou seja, o endereço de memória que 
o ponteiro `pt` está armazenando. Na linha 16, o ((operador de indireção)) `*` é 
utilizado para alterar o valor da variável `tot` por meio do ((ponteiro)) `pt` e, na 
linha 17, esse novo valor é impresso na tela. Execute esse programa no seu 
computador e faça modificações à vontade para você melhor absorver o 
...
.Código fonte
{gitrepo}/blob/master/livro/capitulos/code/cap3/codigo-a.c[code/cap3/codigo-a.c]

[source, c]
.TODO código do professor
----
include::code/cap3/codigo-a.c[]
----
...
conhecido como *((operador de endereço))*, e o operador `*`, normalmente chamado de *((operador de indireção))*. 

O operador de endereço:: é utilizado para se obter o 
endereço de seu operando ou variável a que é aplicado. 

O operador de indireção:: pode ser utilizado de duas formas, a primeira para definição de 
um ponteiro; já a segunda função é retornar o valor ou conteúdo da 
variável para a qual um ponteiro está apontando.
...
analisar aspectos da execução de uma função em um programa em C, como, por 
exemplo, a ((pilha de execução)) de uma ((função)) e transferência de dados para o 
programa principal (função `main`).

As funções em um programa em C são independentes entre si. Como 
consequência, as ((variáveis locais)) definidas dentro do ((escopo de uma função)), 
inclusive os parâmetros de entrada da função, não existem fora dela. Isso 
significa que cada vez que uma função é executada, as suas variáveis locais 
são criadas, e após a sua execução, essas variáveis locais deixam de 
existir. Outra característica básica no conceito de funções é que a 
transferência de dados entre funções é feita com o uso de parâmetros e do 
valor de retorno da função chamada. Ou seja, uma função pode retornar um 
valor para a função que a chamou por meio do comando `return`.
...
execução é analisada usando o esquema ilustrado na Fig. 3.4. Essa figura 
esboça o conceito de ((pilha de execução)) na memória RAM do computador, na 
...
Fig. 3.4: Ilustração da pilha de execução na memória RAM do computador.
...
Continuando o debate, será mostrada agora a _((pilha de execução))_ na memória do 
programa em questão. Será utilizada a notação descrita na Fig. 3.4. 
Resumidamente, a pilha de execução funciona da seguinte forma. Cada variável 
local de uma função de uma função é colocada na pilha de execução. As 
chamar uma função, os parâmetros da função são copiados para a pilha e 
tratados como se fossem variáveis locais da função chamada. Quando a 
execução da função é concluída, a parte da pilha correspondente àquela 
função é liberada. Por isso, não se pode acessar as variáveis locais de 
fora da função em que foram definidas. Perceba na Fig. 3.5a que quando o 
programa inicia, a função `main` inicia com a pilha vazia. Seguindo na 
execução do programa (linhas 7 a 10 do programa), a Fig. 3.5b mostra a 
definição e inicialização das variáveis `soma` e `PAR` na pilha. Com a chamada 
da função (linha 11), a Fig. 3.5c apresenta a variável `PAR` da função 
`f_soma` na pilha de execução (observe que `PAR` inicia com valor 200). A Fig. 
3.5d ilustra a criação/inicialização da variável local `s` dentro da 
função `f_soma`. A Fig. 3.5e mostra a situação da pilha após o laço `while` 
(linha 22), note que agora `PAR` possui valor 0 (zero) e `s` 10100.0 (que é a soma 
dos números pares entre 0 e 200). Nesse caso, veja que a variável `PAR` dentro 
da função `main` continua com seu valor inicial, ou seja, 200. A Fig. 3.5f 
mostra a situação após o retorno da função (linha 23), ou seja, o valor 
retornado pela função `f_soma` é armazenado na variável soma dentro da 
função `main` (linha 11).

Fig. 3.5: Ilustração da pilha de execução na memória RAM de um programa em execução.
...
TIP: A chamada por valor deverá ser usada sempre que não se desejar/precisar 
modificar o valor da variável original que foi chamada.

IMPORTANT: Todas as chamadas em C são feitas por valor. Cabe ao programador 
...
a passagem de *((parâmetros por referência))* para as funções e é necessário o 
uso de ponteiros para esse fim. 
...
.Código fonte
{gitrepo}/blob/master/livro/capitulos/code/cap3/codigo-c.c[code/cap3/codigo-c.c]

[source, c]
----
include::code/cap3/codigo-c.c[]
----

Nesse exemplo não há um problema explícito, pois o resultado da subtração 
é retornado pela função `calculadora` explicitamente (linha 15) e armazenado 
na variável `sub` (linha 8). Um dos problemas da *((passagem de parâmetros por 
...
.Código fonte
{gitrepo}/blob/master/livro/capitulos/code/cap3/codigo-d.c[code/cap3/codigo-d.c]

[source, c]
.Titulo do código
----
include::code/cap3/codigo-d.c[]
----
...
.Código fonte
{gitrepo}/blob/master/livro/capitulos/code/cap3/codigo-e.c[code/cap3/codigo-e.c]

[source, c]
----
include::code/cap3/codigo-e.c[]
----


A diferença entre esse exemplo (forma correta) e o anterior (forma incorreta) 
está nas linhas 4, 8, 14, 15 e 16. 

Na linha 4, o ((protótipo da função)) `calculadora` é definido com duas 
variáveis do tipo inteiro e dois ponteiros para inteiros como parâmetros de 
entrada. Compare com a linha 5 do exemplo anterior (forma incorreta).

Na linha 8, a função `calculadora` continua sendo uma função sem retorno. 
Entretanto, são também passados como parâmetros de entrada os endereços de 
memória das variáveis `sub` e `soma` (i.e., `&sub` e `&soma`). Compare esse trecho do 
código com a linha 10 do exemplo anterior (forma incorreta).

Na linha 14, o código da função `calculadora` começa a ser definido e com os 
dois ponteiros para inteiros como parâmetros de entrada (i.e., `ptr1` e `ptr2`), 
conforme foi definido no protótipo da função na linha 4. Nesse cenário, 
quando a função `calculadora` é chamada na linha 8, os endereços de memória 
das variáveis `sub` e `soma` (i.e., `&sub` e `&soma`) são armazenados nos ponteiros 
`ptr1` e `ptr2`. A partir desse momento, é possível modificar o conteúdo das 
variáveis `sub` e `soma` usando `ptr1` e `ptr2`. Isso é feito nas linhas 15 e 16 para 
se calcular a subtração e soma, respectivamente, usando a funcionalidade do 
// FIXME: seção abaixo
operador de indireção `*` discutida na Seção 3.2. Execute esse código no seu 
computador e veja que agora os valores da subtração e soma são calculados e 
exibidos corretamente.

Fig. 3.6: Ilustração da execução do programa para o cálculo da subtração e soma de dois números inteiros.

A Fig. 3.6 ilustra a pilha de execução da forma correta do programa usando 
ponteiros. Note que na Fig. 3.6c que os ponteiros `ptr1` e `ptr2` recebem os 
endereços de memória das variáveis `sub` e `soma`, respectivamente. Com isso, 
agora é possível alterar o conteúdo de `sub` e `soma` usando esses ponteiros, 
mesmo dentro de uma função em que `sub` e `soma` não são definidas. Isso é 
mostrado na Fig. 3.6d, observe que os ponteiros são utilizados para se 
computar a subtração e soma. A Fig. 3.7e apresenta a situação da pilha 
após a função `calculadora` ser executada. Veja que as variáveis 
locais `x`, `y`, `*ptr1` e `*ptr2` são desempilhadas.
